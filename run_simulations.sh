...
#mpirun -n 6 python -m mpi4py.futures -m nested.analyze --config-file-path=config/optimize_config_6b_FF_Inh+indirect_FB_Inh+FB_Exc_multiple_seeds.yaml --param-file-path=config/20211110_model_params.yaml --model-key=6b --framework=mpi --export --export_dynamics --export-file-path=data/dentate_optimization_dynamics.hdf5
