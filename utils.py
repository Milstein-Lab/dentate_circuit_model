...
def export_model_slice_data(export_file_path, description, weight_seed, model_config_dict, weight_dict,
    ...
    :param weight_history_dict: nested dict of ndarray of float
    """
            ...
            for pre_pop in weight_dict[post_pop]:
                ...
                for key, value in weight_config_dict[post_pop][pre_pop].items():
                    if isinstance(value, dict):
                        post_group[pre_pop].attrs[key] = str(value)
                    else:
                        post_group[pre_pop].attrs[key] = value
        
        if weight_history_dict is not None:
            group = model_seed_group.create_group('weight_history')
            for post_pop in weight_history_dict:
                post_group = group.create_group(post_pop)
                for pre_pop in weight_history_dict[post_pop]:
                    post_group.create_dataset(pre_pop, data=weight_history_dict[post_pop][pre_pop])
